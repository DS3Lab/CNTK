                                      ...
                                      IDataReader* validationSetDataReader)
{
    ...
    std::vector<std::unique_ptr<Matrix<ElemType>>> estimated_weight;
    estimated_weight.reserve(8);
    std::vector<std::unique_ptr<Matrix<ElemType>>>& y = estimated_weight;
    ...
    for (int i = startEpoch; i < (int) m_maxEpochs; i++) // TODO: why is this an int, and not a size_t?
    {
        ...
        //222
        //printf("[%d] epochs: %d, MBs: %d\n", (int) m_mpi->CurrentNodeRank(), i, (int)totalMBsSeen);

        // if(i > 0){
        //     printf("[%d] epoch: %d, value: %f\n", (int) m_mpi->CurrentNodeRank(), i, y[0]->GetValue(0,0));
        // }

        //printf("[%d] epochnumber: %d\n", (int) m_mpi->CurrentNodeRank(), i);
...
template <class ElemType>
size_t SGD<ElemType>::TrainOneEpoch(//std::vector<std::unique_ptr<Matrix<ElemType>>>& y,
                                    ComputationNetworkPtr net,
                                    ComputationNetworkPtr refNet,
    ...
    for (;;)
    {
        ...
        if (!useGradientAggregation)
        {
         ...
         else
         {
            ...
            // int headerbegin, headerend;
            // headerbegin = time(NULL);
            //shaoduo: exchange header infoemation
        ...
        if ((aggregateNumSamples > 0) && (learnRatePerSample > m_minLearnRate * 0.01))
        {
            ...
            if (learnParamsWeights.size() == 0)
            {
                ...
                for (auto nodeIter = learnableNodes.begin(); nodeIter != learnableNodes.end(); nodeIter++)
                {
                    ...
                    if (node->IsParameterUpdateRequired())
                    {
                        ...
                        // {
                        //     //Matrix<ElemType>* currParamsValues = &(node->Value());
                        //     currParamsGradient->Resize(currParamsWeight->GetNumRows(), currParamsWeight->GetNumCols());
                        // }
            ...
            //initialize y in first eopch in first iteration
            if(y.size() == 0){
            ...
            //get current weight
            std::unique_ptr<Matrix<ElemType>> x1;
            x1.reset();
            x1.reset(new (std::nothrow) Matrix<ElemType>(1, numofWeights, learnParamsWeights[0]->GetDeviceId()));
            size_t offset1 = 0;
            for (int i = 0; i < learnParamsWeights.size(); i++)
            {
                x1->ColumnSlice(offset1, learnParamsWeights[i]->GetNumElements()).AssignValuesOf(learnParamsWeights[i]->Reshaped(1, learnParamsWeights[i]->GetNumElements()));
                offset1 += learnParamsWeights[i]->GetNumElements();
            }

            //averaging weights
            std::unique_ptr<Matrix<ElemType>> x_half;
            x_half.reset();
            x_half.reset(new (std::nothrow) Matrix<ElemType>(1, numofWeights, learnParamsWeights[0]->GetDeviceId()));
            ...
            for(int i = 0; i <= indexNeighbor.size(); i++){
                ...
                if(i < indexNeighbor.size()){
                    Matrix<ElemType>::ScaleAndAdd(weight[myrank][indexNeighbor[i]], *y[i], *x_half);
                }else if(i == indexNeighbor.size()){
                    Matrix<ElemType>::ScaleAndAdd(weight[myrank][myrank], *x1, *x_half);
                }
            ...
            for (int i = 0; i < learnParamsWeights.size(); i++)
            {
                ...
                learnParamsWeights[i]->AssignValuesOf(x_half->ColumnSlice(offset2, learnParamsWeights[i]->GetNumElements()).Reshaped(learnParamsWeights[i]->GetNumRows(), learnParamsWeights[i]->GetNumCols()));
            ...
            for (auto nodeIter = learnableNodes.begin(); nodeIter != learnableNodes.end(); nodeIter++, smoothedGradientIter++, smoothedCountIter++)
            {
                ...
                if (node->IsParameterUpdateRequired())
                {
                    ...
                    //         mid_weight->SetValue(0, row * numCols + col, dynamic_pointer_cast<ComputationNode<ElemType>>(node)->Value().GetValue(row, col))
            ...
            //get new weight in x_half
            size_t offset3 = 0;
            for (int i = 0; i < learnParamsWeights.size(); i++)
            {
                x_half->ColumnSlice(offset3, learnParamsWeights[i]->GetNumElements()).AssignValuesOf(learnParamsWeights[i]->Reshaped(1, learnParamsWeights[i]->GetNumElements()));
                offset3 += learnParamsWeights[i]->GetNumElements();
            }


            beta = (float) 2.0/(totalMBsSeenBefore + numMBsRun + 1 - epochNumber);
            //beta = (float) 2.0/(numMBsRun + 1 - epochNumber);

            
            // std::unique_ptr<Matrix<ElemType>> z;
            // z.reset();
            // z.reset(new (std::nothrow) Matrix<ElemType>(1, numofWeights, learnParamsWeights[0]->GetDeviceId()));
            // Matrix<ElemType>::ScaleAndAdd((1.0 - 1.0/beta), *send_buffer, *z);
            // Matrix<ElemType>::ScaleAndAdd((1.0/beta), *mid_weight, *z);

            //printf("[%d] test2\n", myrank);

            //calculate z (which is x1)
            Matrix<ElemType>::ScaleAndAdd((1.0/beta), *x_half, (1.0 - 1.0/beta), *x1);


            //find out max and min
            ElemType max = x1->GetValue(0, 0), min = x1->GetValue(0, 0);
            int index_max = 0, index_min = 0;
            for(int j = 0; j < numofWeights; j++){

                if (x1->GetValue(0, j) > max){
                    max = x1->GetValue(0, j);
                    //index_max = j;
                }
                if(x1->GetValue(0, j) < min){
                    min = x1->GetValue(0, j);
                    //index_min = j;
                }
            }
            ...
            unsigned char **send_buffer = new unsigned char*[indexNeighbor.size()];
            for (int i = 0; i < indexNeighbor.size(); ++i)
            {
                send_buffer[i] = new unsigned char[numofWeights];
            }
            ...
            for(int j = 0; j < numofWeights; j++){
                ...
                ElemType d = (x1->GetValue(0, j) - min) / ((max - min) / 255.0);
            ...
            // printf("[%d], 0:%d, max:%d, min:%d\n", myrank, send_buffer[0], 
            //     send_buffer[index_max], send_buffer[index_min]);
            ...
            request.push_back(MPI_Request());
            m_mpi->Isend(send_buffer2, 2, MPIWrapper::GetDataType(x1->Data()), indexNeighbor[neighbor], 0, &request.back());

            request.push_back(MPI_Request());
            m_mpi->Irecv(recv_buffer[neighbor], numofWeights, MPI_UNSIGNED_CHAR, indexNeighbor[neighbor], 0, &request.back());
            
            request.push_back(MPI_Request());
            m_mpi->Irecv(recv_buffer2[neighbor], 2, MPIWrapper::GetDataType(x1->Data()), indexNeighbor[neighbor], 0, &request.back());
            ...
            for(int i = 0; i < indexNeighbor.size(); i++){ //for each y
                ...
                ElemType unit;
                unit = (recv_buffer2[i][0] - recv_buffer2[i][1]) / 255.0; 
                std::vector<ElemType> z(numofWeights);

                for(int j = 0; j < numofWeights; j++){ //un-compress z to x1

                    ElemType unexpress;
                    unexpress = recv_buffer2[i][1] + recv_buffer[i][j] * unit;
                    x1->GetValue(0, j);
                    x1->SetValue(0, j, unexpress);
                }

                Matrix<ElemType>::ScaleAndAdd(beta, *x1, (1 - beta), *y[i]);
